...
class HCModel17(CaptionModel):
    def __init__(self, opt, ix2glove=None):
        super(HCModel17, self).__init__()
    ...
    def _prepare_feature(self, human_rela_feats, other_feats, human_rela_masks, other_masks, body_feats, body_masks, fc_feats, att_feats, att_masks):
    ...
    def splitAttFeats(self, att_feats, att_masks, human_related_weight):

        # human_weight = human_weight.view(-1, 3, human_weight.shape[-1])
        human_related_weight = human_related_weight.view(-1, 3, human_related_weight.shape[-1])
        human_masks = (human_related_weight > self.weight_thresh)
        tmp = torch.sum(human_related_weight, dim=1)
        ...
        return human_rela_feats, other_feats, human_rela_masks, other_masks


    def _forward(self, fc_feats, att_feats, seq, body_feats, part_feats, body_masks, part_masks, att_masks=None):
        human_related_weight, human_feats = self.head(fc_feats, att_feats, body_feats, part_feats, att_masks, body_masks, part_masks)

        human_rela_feats, other_feats, human_rela_masks, other_masks = self.splitAttFeats(att_feats, att_masks, human_related_weight)
        ...
        p_att_feats1, att_masks1, att_feats_ori, p_att_feats_ori, att_masks_ori = \
            self._prepare_feature(human_rela_feats, other_feats, human_rela_masks, other_masks, body_feats, body_masks, fc_feats, att_feats, att_masks)
    ...
    def _sample_beam(self, fc_feats, att_feats, body_feats, part_feats, body_masks, part_masks, att_masks=None, opt={}):
        ...
        human_related_weight, human_feats = self.head(fc_feats, att_feats, body_feats, part_feats, att_masks,
                                                    body_masks, part_masks)
        human_rela_feats, other_feats, human_rela_masks, other_masks = self.splitAttFeats(att_feats, att_masks,
                                                                                          human_related_weight)
        # other_feats, other_masks = att_feats, att_masks

        fc_feats, att_feats, p_att_feats, att_masks, att_feats1, \
        p_att_feats1, att_masks1, att_feats_ori, p_att_feats_ori, att_masks_ori = \
            self._prepare_feature(human_rela_feats, other_feats, human_rela_masks,
                                  other_masks, body_feats, body_masks, fc_feats, att_feats, att_masks)
...
class HakeModule2(nn.Module):
    ...
    def forward(self, fc_feats, att_feats, body_feats, part_feats, att_masks, body_masks, part_masks):
        ...
        h_att = human_feats.view(-1, human_feats.shape[-1])
        out_att, out_weight = self.attention(h_att, ex_att_feats, ex_p_att_feats, ex_att_masks)
        # obj_att = obj_att.view(human_feats.shape)
        ...
        # part_out = part_out.view(part_out.shape[0], -1, part_out.shape[-1])
        # human_weight = human_weight.view(body_masks.shape[0], body_masks.shape[1], -1)
        out_weight = out_weight.view(body_masks.shape[0], body_masks.shape[1], -1)
        # human_weight = human_weight * body_masks.unsqueeze(-1)
        out_weight = out_weight * body_masks.unsqueeze(-1)

        return out_weight, human_feats
